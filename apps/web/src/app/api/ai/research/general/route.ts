...
export async function POST(request: NextRequest) {
  const { name, organisation, linkedinHandle } = await request.json();
  ...
  const identifierLines: string[] = [`- Full name: ${personName}`];
...
- Summary of their LinkedIn profile${handle ? ` (https://www.linkedin.com/in/${handle})` : ''}
- Recent posts, articles, or comments they have made publicly
...
---
Focus strictly on ${personName} as an individual. Every point should be traceable to this specific person.`;
